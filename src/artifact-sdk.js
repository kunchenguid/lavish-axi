...
export function createArtifactSdk(
  ...
  // Operator theme tokens for the annotation card, threaded from the server via
  // createSdkJs. /chrome.css restyles the rest of the editor, but it can never reach
  // this card: it renders in a shadow root inside the sandboxed artifact iframe. Only
  // custom properties arrive here (theme-css.js drops everything else), and they are
  // appended to the card's own :host token block so they override it by cascade order.
  // The operator stylesheet is deliberately NOT injected into the artifact document -
  // the artifact carries its own theme and the two would fight.
  const THEME_TOKENS = typeof options.themeTokens === "string" ? options.themeTokens : "";
  // The annotate-mode outline is set on artifact elements, outside the block that
  // declares those tokens, so it needs a resolved literal rather than a var().
  const THEME_ACCENT = (typeof options.themeAccent === "string" && options.themeAccent) || "#f4c95d";
  ...
  function highlightElement(el) {
    if (!el) return;
    el.style.outline = "var(--lavish-annotate-outline,2px solid " + THEME_ACCENT + ")";
  ...
  function setAnnotationMode(enabled) {
    ...
    if (annotationMode && !style) {
      ...
      // Only the resolved accent literal is written here, never the operator
      // stylesheet - artifacts still override --lavish-accent themselves.
      style.textContent =
        ":root{--lavish-accent:" +
        THEME_ACCENT +
