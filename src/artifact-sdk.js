...
export function createArtifactSdk(
  ...
  function queuePrompt(prompt, options = {}) {
    ...
    /** @type {{ id: string, uid: string, prompt: string, selector: string, tag: string, text: string, target?: unknown, attachments?: Array<{ id: string, name?: string }>, _lavishQueueKey?: string }} */
    const item = {
  ...
  function revealElement(selector) {
    ...
    // The marker is position:fixed but scrollIntoView is asynchronous under `behavior: "smooth"`,
    // so a rect read now is pre-scroll and would strand the box over whatever the element used to
    // be next to - the "ghost box" that only looked right once the element was already in view.
    // Re-read the rect every frame for the pulse's lifetime instead, which also keeps the marker
    // on target through any layout shift the scroll triggers.
    const track = () => {
