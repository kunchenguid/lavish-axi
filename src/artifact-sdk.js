...
export function createArtifactSdk(
  ...
  function setAnnotationMode(enabled) {
    ...
    if (annotationMode && !style) {
      ...
      style.textContent =
        ":root{--lavish-accent:#f4c95d;--lavish-annotate-outline:2px solid var(--lavish-accent);--lavish-annotate-offset:2px}[data-lavish-action],[data-lavish-action] *{cursor:pointer!important}";
  ...
  function closeCard() {
    if (shadow) {
      for (const el of [...shadow.querySelectorAll(".lavish-annotation-card,.lavish-context-menu")]) el.remove();
    }
  ...
  function copySelectedText(text) {
    ...
    try {
      ...
      // Clipboard access differs across sandbox and browser versions; try the
      // asynchronous API below without disrupting the current selection.
    }
  ...
  function showAnnotationMenu(target, options = {}) {
    const root = ensureShadow();
    closeCard();
    ...
    menu.className = "lavish-context-menu";
    menu.setAttribute("role", "menu");
    menu.addEventListener("mousedown", (event) => event.preventDefault());

    if (c.tag === "text") {
      const selectedText = document.getSelection()?.toString() || c.text;
    ...
    annotateButton.className = "lavish-annotate-action";
    annotateButton.textContent = c.tag === "text" ? "Annotate selection" : "Annotate element";
    annotateButton.onclick = () => showAnnotationCard(target, options);
    menu.appendChild(annotateButton);
    root.appendChild(menu);

    const left = Math.min(Math.max(8, Number(options.x) || 0), window.innerWidth - menu.offsetWidth - 8);
    const top = Math.min(Math.max(8, Number(options.y) || 0), window.innerHeight - menu.offsetHeight - 8);
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      let c = textSelectionContext(document.getSelection());
      if (c && c.range.intersectsNode) {
        try {
          if (!c.range.intersectsNode(event.target)) c = null;
        } catch {
          c = null;
        }
      }
      ...
      showAnnotationMenu(c?.element || event.target, {
        context: c || undefined,
        range: c?.range,
        x: event.clientX,
        y: event.clientY,
