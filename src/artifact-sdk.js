...
export function createArtifactSdk(
  ...
  function showAnnotationCard(target, options = {}) {
    ...
    card.innerHTML =
      ...
      '</div><div class="lavish-row"><button class="lavish-cancel" type="button">Cancel</button>' +
      '<span class="lavish-spacer"></span><button class="lavish-queue" type="button">Queue</button>' +
      '<button class="lavish-send" type="button">Send</button></div>';
    ...
    if (!textarea || !cancelButton || !queueButton || !sendButton || !attachmentsList || !attachButton || !attachInput)
      return;
    ...
    sendButton.onclick = () => {
      // Same guard as the Cmd/Ctrl+Enter path: an empty card closes without sending.
      const sendNow = !!textarea.value.trim() || attachments.hasReady();
