...
export const POLL_WAKE_PATH_RULES = Object.freeze([
  "Keep the poll in the foreground by default and let it return the feedback directly to the agent. A background poll is allowed only through a harness-native tracked background-job facility whose completion result is guaranteed to resume or notify the same agent.",
  "Never use `nohup`, shell `&`, `disown`, redirected fire-and-forget processes, or a detached terminal without an explicit verified callback. If the harness has no completion-aware background facility, use the foreground poll or first wire a verified wake callback into the surrounding supervisor.",
  "Do not tell the user the artifact is being monitored until that wake path is live. When the foreground poll is represented as a tracked command or process, resume that same tracked process with the longest practical blocking wait the harness supports; do not poll it on short fixed intervals.",
  "Waiting is transport, not reasoning work: an empty wait result must not trigger fresh reasoning or narration, and do not substitute agent-status polling for the artifact feedback wait. If the poll gets killed or times out anyway, just re-run it - queued feedback is never lost.",
]);
export const MODEL_USAGE_RULE =
