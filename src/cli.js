...
function pollExecutionGuidance({ waitForBackground = false, agent = "generic" } = {}) {
  ...
  return `${POLL_BACKGROUND_TASK_GUIDANCE}${waitForBackground ? " and wait for it to finish" : ""}; if it gets killed or times out anyway, just re-run it - queued feedback is never lost.`;
}
...
export function createHomeOutput({ bin, sessions, includeSessions = true, agent = "generic" }) {
  return {
    ...
    help: [
      ...
      `Run \`lavish-axi poll <html-file>\` to long-poll for user feedback or browser-reported layout_warnings. Feedback is redelivered with the same delivery_id until you process it and pass \`--ack <delivery_id>\` on the next poll; this preserves feedback if a poll result is abandoned. The poll stays silent until the user sends feedback, ends the session, or the real browser reports fresh layout_warnings, so leave it running - never kill it. Fix and re-check fresh error-severity layout_warnings before involving the human; if the poll says every current warning is persistent or low-severity, proceed with a note instead of looping. ${pollExecutionGuidance({ agent })} Acknowledge final feedback before stopping; when the next poll reports the session ended, stop polling and do not reopen it uninvited - deliver remaining updates in this conversation instead`,
...
export function createOpenOutput({ file, url, status, agent = "generic" }) {
  return {
    ...
    next_step: `Do not respond to the user just yet. Now you must run \`lavish-axi poll ${file}\`. This command long-polls until the user sends feedback, ends the session, or the real browser reports layout_warnings from the in-iframe layout audit, and it stays silent the whole time - that is normal, never kill it. If layout_warnings arrive, follow the poll response's next_step: fix and re-check fresh error-severity overflow or clipped-text findings before involving the human, but persistent or low-severity warnings may be surfaced with a note when the cause is not obvious. Do not pass --timeout-ms during normal agent use. ${pollExecutionGuidance({ waitForBackground: true, agent })} After applying feedback, run \`lavish-axi poll ${file} --ack <delivery_id> --agent-reply "<message for the user>"\` without --timeout-ms to acknowledge that processed delivery, show your response in Lavish Editor, and wait for more feedback. If the user ends the session, acknowledge the final delivery before stopping; do not reopen it by re-running \`lavish-axi ${file}\` unless the user asks for further review or something genuinely important needs their visual attention. When reopening is warranted, run \`lavish-axi ${file} --reopen\`.`,
  };
}
...
async function pollCommand(args) {
  const file = firstPositionalArg(args, ["--ack", "--agent-reply", "--timeout-ms"]);
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `lavish-axi poll <html-file>`"]);
  }
  const absolute = await canonicalFile(file);
  const baseUrl = await ensureServer();
  const deliveryId = flagValue(args, "--ack");
  if (deliveryId && !/^[0-9a-f]{16}$/.test(deliveryId)) {
    throw new AxiError("--ack must be a 16-character delivery id", "VALIDATION_ERROR", [
      `Use the delivery_id returned by \`lavish-axi poll ${absolute}\``,
    ]);
  }
  if (deliveryId) {
    await postJson(
...
export function createPollOutput({ file, response, agent = "generic" }) {
  ...
  if (response.status === "feedback") {
    ...
    return {
      ...
      ...(response.delivery_id ? { delivery_id: response.delivery_id } : {}),
...
function createFeedbackNextStep(
  ...
  if (sessionEnded) {
    ...
    if (endedBy === "user") {
      if (deliveryId) {
        return `${layoutNote}${whiteboardNote}This was the last feedback before the user ended the session. After applying it, run \`lavish-axi poll ${file} --ack ${deliveryId} --agent-reply "<message for the user>"\`; it will acknowledge this final delivery and report the ended session, then stop polling and do not reopen it. Only run \`lavish-axi ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
      }
    ...
    if (deliveryId) {
      return `${layoutNote}${whiteboardNote}This was the last feedback before the Lavish Editor session ended. After applying it, run \`lavish-axi poll ${file} --ack ${deliveryId} --agent-reply "<message for the user>"\`; it will acknowledge this final delivery and report the ended session, then stop polling. Run \`lavish-axi ${file}\` to open a fresh session only if the user needs further visual review.`;
    }
  ...
  const acknowledgeFlag = deliveryId ? ` --ack ${deliveryId}` : "";
  return `${layoutPrefix}${whiteboardNote}Do not respond to the user just yet. Now you must run \`lavish-axi poll ${file}${acknowledgeFlag} --agent-reply "<message for the user>"\` without --timeout-ms unless the user ended the session. The poll waits silently until the user sends more feedback, ends the session, or reports fresh layout_warnings - never kill it. ${pollExecutionGuidance({ agent })}`;
}
...
function createTopLevelHelp({ agent = "generic" } = {}) {
  return `lavish-axi - Lavish Editor AXI\n\nUsage:\n  lavish-axi\n  lavish-axi <html-file> [--no-open] [--no-gate] [--reopen]\n  lavish-axi poll <html-file> [--ack <delivery-id>] [--agent-reply "..."]\n  lavish-axi end <html-file>\n  lavish-axi export <html-file> [--out <path>]\n  lavish-axi share <html-file> [--password <pw>] [--token <t>]\n  lavish-axi stop\n  lavish-axi playbook [playbook_id]\n  lavish-axi design\n  lavish-axi setup hooks\n\n${DESIGN_SYSTEM_HINT}\n\nNote: poll long-polls indefinitely by default until the user sends feedback, ends the session, or the browser reports fresh layout_warnings, staying silent while it waits - never kill it. Feedback is redelivered until the agent processes it and acknowledges its delivery_id on the next poll. Fix and re-check fresh error-severity layout_warnings before involving the human; persistent or low-severity findings may be surfaced with a note when the cause is not obvious. Do not pass --timeout-ms during normal agent use; it is for tests and debugging only. ${pollExecutionGuidance({ agent })} When the user ends a session from the browser, acknowledge final feedback, then stop polling and do not reopen it uninvited - pass --reopen to <html-file> only when the user asks for further review or something important needs their visual attention.\n\n`;
}
...
function createCommandHelp({ agent = "generic" } = {}) {
  return {
    ...
    poll: `Usage: lavish-axi poll <html-file> [--ack <delivery-id>] [--agent-reply "..."]\n\nThis command long-polls indefinitely for queued user prompts and browser-reported layout_warnings, then returns them to the agent. Feedback is redelivered until you process it and pass its delivery_id with --ack on the next poll. It stays silent while it waits - that is normal, never kill it. Fix and re-check fresh error-severity layout_warnings before involving the human; persistent or low-severity findings may be surfaced with a note when the cause is not obvious. Do not pass --timeout-ms during normal agent use; it is for tests and debugging only. ${pollExecutionGuidance({ waitForBackground: true, agent })} Use --agent-reply after applying prior feedback to display your response in Lavish Editor before waiting again. When status is ended, stop polling and do not reopen the session uninvited - deliver remaining updates directly in this conversation instead.\n`,
