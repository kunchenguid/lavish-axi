...
export function createHomeOutput({ bin, sessions, includeSessions = true }) {
  return {
    ...
    help: [
      ...
      'Mermaid diagrams in artifacts render as embedded, editable Excalidraw whiteboards in the browser (click a diagram to unlock editing; a Fullscreen action opens it over the whole viewport) - flowchart, sequence, class, ER, and state diagrams convert to editable shapes; other types embed as an image to draw on. Standalone and exported copies still render plain Mermaid. When the user queues whiteboard feedback, poll returns a prompt with tag "whiteboard" carrying a bounded edit summary plus local scenePath (.excalidraw JSON) and previewPath (PNG) files - read the summary first, open the files only when needed, then apply the edits by updating the Mermaid source in the artifact (never try to write the scene back)',
