...
export function createHomeOutput({ bin, sessions, includeSessions = true }) {
  return {
    ...
    help: [
      ...
      "Run `lavish-axi stop` to shut down the background server (it also self-stops when idle or after the last session ends)",
...
const COMMAND_HELP = {
  ...
  stop: `Usage: lavish-axi stop [--port <port>]\n\nShut down the background Lavish Editor server. The server also stops itself when no browser or poll has been connected for a while (LAVISH_AXI_IDLE_TIMEOUT_MS, default 30m) and immediately when the last session ends.\n`,
