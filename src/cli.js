...
export function createHomeOutput({ bin, sessions, includeSessions = true }) {
  return {
    ...
    help: [
      ...
      'Mermaid diagrams in artifacts get an "Edit as whiteboard" button (on hover) that opens them as an editable Excalidraw whiteboard in the browser - flowchart, sequence, class, ER, and state diagrams convert to editable shapes; other types open as an image to draw on. When the user queues whiteboard feedback, poll returns a prompt with tag "whiteboard" carrying a bounded edit summary plus local scenePath (.excalidraw JSON) and previewPath (PNG) files - read the summary first, open the files only when needed, then apply the edits by updating the Mermaid source in the artifact (never try to write the scene back)',
