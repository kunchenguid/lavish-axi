...
export const CODEX_POLL_WAKE_PATH_GUIDANCE =
  "Codex detected: use one `functions.exec` cell as the callback owner. Inside that JavaScript, start `lavish-axi poll` with `tools.exec_command`; if it yields a session_id, call `yield_control()` once, keep the longest practical blocking `tools.write_stdin` waits inside that cell until the process exits, then call `notify(...)` only with the final poll output. This keeps waiting model-free and delivers completion to the same agent. Do not make the model call `functions.wait` or `write_stdin` in a loop; an empty wait must not trigger fresh reasoning or narration, and do not substitute agent-status polling for the artifact feedback wait. Do not call `wait_agent`, use a separate or cheaper polling subagent, or start a second `lavish-axi poll`. Never start a detached `codex app-server` or call `thread/resume` for the active `CODEX_THREAD_ID`; that creates a second runtime owner instead of a callback into this agent. If code-mode callbacks are unavailable, keep the original foreground long poll attached to the active turn with the longest supported blocking wait.";
