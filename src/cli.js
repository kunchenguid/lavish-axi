...
function serverConnectionError() {
  return new AxiError("Lavish Editor server connection failed", "SERVER_ERROR", [
    "Run `lavish-axi server --verbose` or inspect `$XDG_DATA_HOME/lavish-axi/server.log` (`~/.local/share/lavish-axi/server.log` by default, or `LAVISH_AXI_STATE_DIR/server.log` when set) for server startup or crash diagnostics",
