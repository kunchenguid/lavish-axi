...
export function createHomeOutput({ bin, sessions, includeSessions = true, agent = "generic" }) {
  return {
    ...
    help: [
      ...
      `Run \`lavish-axi poll <html-file>\` to wait for user feedback or browser-proven severe layout failures. Feedback is redelivered with the same delivery_id until you process it and pass \`--ack <delivery_id>\` on the next poll; this preserves feedback if a poll result is abandoned. It long-polls and stays silent until the user sends feedback, ends the session, or the real browser proves meaningful content is inaccessible or unusable, so leave it running - never kill it. Repair and re-check every returned layout failure before involving the human; cosmetic, intentional, transient, tiny, and uncertain observations stay silent. ${pollExecutionGuidance({ agent })} Acknowledge final feedback before stopping. ${POLL_SEND_AND_END_RULE}`,
...
function createFeedbackNextStep(
  ...
  if (sessionEnded) {
    ...
    if (endedBy === "user") {
      ...
      if (deliveryId) {
        return `${layoutNote}${whiteboardNote}This was the last feedback before the user ended the session. After applying it, run \`lavish-axi poll ${file} --ack ${deliveryId} --agent-reply "<message for the user>"\`; it will acknowledge this final delivery and report the ended session, then stop polling and do not reopen it.${reopenNote}`;
      }
...
function createTopLevelHelp({ agent = "generic" } = {}) {
  return `lavish-axi - Lavish Editor AXI\n\nUsage:\n  lavish-axi\n  lavish-axi <html-file> [--no-open] [--no-gate] [--reopen]\n  lavish-axi poll <html-file> [--ack <delivery-id>] [--agent-reply "..."]\n  lavish-axi end <html-file>\n  lavish-axi export <html-file> [--out <path>]\n  lavish-axi share <html-file> [--password <pw>] [--token <t>]\n  lavish-axi stop\n  lavish-axi playbook [playbook_id]\n  lavish-axi design\n  lavish-axi setup hooks\n\n${DESIGN_SYSTEM_HINT}\n\nNote: poll long-polls indefinitely by default until the user sends feedback, ends the session, or the browser proves a severe layout failure, staying silent while it waits - never kill it. Feedback is redelivered until the agent processes it and acknowledges its delivery_id on the next poll. Repair and re-check every returned layout failure before involving the human; cosmetic and uncertain observations are never returned. Do not pass --timeout-ms during normal agent use; it is for tests and debugging only. ${pollExecutionGuidance({ agent })} When the user ends a session from the browser, acknowledge final feedback, then stop polling and do not reopen it uninvited. ${POLL_SEND_AND_END_RULE}\n\n`;
}
