...
export function createChromeHtml(
  ...
  const sessionJson = jsonScript({
    ...
    // The chrome client is served raw and cannot import the theme module, so the picker and the
    // annotation card get their values here, the same way the revision palette does.
    chromeThemes: serializeChromeThemes(),
    defaultChromeTheme: DEFAULT_CHROME_THEME,
    chromeThemeStorageKey: CHROME_THEME_STORAGE_KEY,
  });
  ...
  const chromeThemes = serializeChromeThemes();
  const defaultTheme = chromeThemes.find((theme) => theme.id === DEFAULT_CHROME_THEME) || chromeThemes[0];
  const themeSwatches = chromeThemes
    .map(
      (theme) =>
        `<button class="theme-swatch" id="themeSwatch-${theme.id}" type="button" role="radio" aria-checked="${theme.id === DEFAULT_CHROME_THEME}" aria-label="${escapeHtml(`${theme.name} · ${theme.description}`)}" title="${escapeHtml(`${theme.name} · ${theme.description}`)}" style="--swatch-ground: ${theme.swatch.ground}; --swatch-accent: ${theme.swatch.accent}"><span class="theme-swatch-dot" aria-hidden="true"></span></button>`,
    )
    .join("");
  return `<!doctype html>
<html>
...
const sanitizeChromeThemeTokens=${sanitizeChromeThemeTokens.toString()};
