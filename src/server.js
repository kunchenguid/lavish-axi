...
import { canonicalFile, getThemePreference, sessionKey, SessionStore } from "./session-store.js";
...
export async function serve({
  ...
  app.get("/session/:key", async (req, res, next) => {
    try {
      ...
      await watchSession(session, watchers, events, logEvent);
      const themePref = await getThemePreference(store);
      res
        .type("html")
        .send(createChromeHtml(session, { layoutGateEnabled: shouldEnableLayoutGate(req.query || {}), themePref }));
    } catch (error) {
      next(error);
    }
  });
...
export function createChromeHtml(session, { layoutGateEnabled = true, themePref = "system" } = {}) {
  ...
  // themePref is a validated enum (system|light|dark); safe to embed as an attribute.
  return `<!doctype html>
<html data-theme-pref="${escapeHtml(themePref)}">
<head>
<meta charset="utf-8">
