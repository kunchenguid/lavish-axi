...
import {
  clampPanelWidth as clampPanelWidthHelper,
  loadStoredPanelWidth as loadStoredPanelWidthHelper,
  PANEL_DEFAULTS,
  PANEL_STORAGE_KEY,
  resolveDefaults,
  savePanelWidth as savePanelWidthHelper,
} from "./panel-width.js";
...
function serializePanelWidthForBrowser() {
  // Inline the same pure helpers the chrome uses to clamp/persist the panel
  // width so they share one source of truth with the unit tests in
  // test/panel-width.test.js. Wrapped in an IIFE that exposes them on
  // `globalThis.LavishPanelWidth`, mirroring how the artifact SDK is inlined
  // (see `createSdkJs`).
  return `(() => {
const PANEL_DEFAULTS=${JSON.stringify(PANEL_DEFAULTS)};
const PANEL_STORAGE_KEY=${JSON.stringify(PANEL_STORAGE_KEY)};
const resolveDefaults=${resolveDefaults.toString()};
const clampPanelWidth=${clampPanelWidthHelper.toString()};
const loadStoredPanelWidth=${loadStoredPanelWidthHelper.toString()};
const savePanelWidth=${savePanelWidthHelper.toString()};
globalThis.LavishPanelWidth={ PANEL_DEFAULTS, PANEL_STORAGE_KEY, resolveDefaults, clampPanelWidth, loadStoredPanelWidth, savePanelWidth };
})();`;
}
...
<script>${serializePanelWidthForBrowser()}</script>
