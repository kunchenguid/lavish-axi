...
export function createChromeHtml(
  ...
  const sessionJson = jsonScript({
    ...
    // The legend swatch has to paint the same texture the artifact highlight
    // does, and chrome-client.js is served raw so it cannot import it. Ship the
    // one definition rather than letting a second copy drift out of step.
...
const REVISION_PALETTE=${JSON.stringify(REVISION_PALETTE)};
const REVISION_PATTERN_CSS=${JSON.stringify(REVISION_PATTERN_CSS)};
const revisionTruncate=${revisionTruncate.toString()};
...
const collectRevisionMarkedElements=${collectRevisionMarkedElements.toString()};
