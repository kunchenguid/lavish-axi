...
\`\`\``,
  ...
  },
  {
    ...
    design_rules: [
      ...
      'When you revise an artifact the user already reviewed, declare what changed: embed a `<script type="application/json" data-lavish-revisions>` registry of your edit batches and put data-lavish-revision="<id>" on each element you touched, space-separated when an element changed across several batches so the most recent one wins. Lavish then shows a Revisions legend that highlights those elements with a color plus its own border style and background pattern, never color alone. Six such combinations exist and are assigned in registry order, so a seventh revision reuses the first one\'s appearance.',
      'Keep each revision id addressable from that space-separated attribute: no whitespace, no repeat of an earlier id, and never the reserved "*". Unaddressable and duplicate entries are skipped while the rest of the registry still renders.',
    ...
    lavish_notes: [
      ...
      'A revised artifact should carry its own edit history: `<script type="application/json" data-lavish-revisions>[{"id": "1", "label": "Initial draft", "timestamp": "2026-07-29T10:00:00Z", "summary": "First pass layout", "sections": ["Hero"]}]</script>` plus `<section data-lavish-revision="1">...</section>` on the elements that batch changed. Only label is worth writing by hand; timestamp, summary, and sections are optional. An artifact with no registry renders exactly as before, with no legend.',
