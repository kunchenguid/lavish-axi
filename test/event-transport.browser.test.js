...
function observeAfterNavigation(args, env, timeout = 10_000) {
  ...
  while (Date.now() < deadline) {
    ...
    if (!output.includes("Execution context was destroyed")) {
      assert.equal(result.status, 0, `chrome-devtools-axi ${args.join(" ")}\n${output}`);
    }
  }
  assert.fail(`chrome-devtools-axi ${args.join(" ")}\n${output}`);
}
  ...
  async () => {
    ...
    try {
      ...
      for (let index = 0; index < sessions.length; index += 1) {
        ...
        const observed = observeAfterNavigation(
          [
            "eval",
