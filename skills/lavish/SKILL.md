...
- Run `npx -y lavish-axi screenshot <html-file> [--out <path>] [--width <px>]` to capture a full-page (long) PNG of the rendered artifact with headless Chrome - the image spans the document's full scroll height, not just the viewport. It renders the file directly, so it needs no Lavish server
