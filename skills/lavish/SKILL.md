...
- Run `npx -y lavish-axi tailnet` to serve live sessions to the user's other Tailscale devices (phone, tablet), private to their tailnet - session output then includes a tailnet_url to hand the user; `--off` stops it. Public exposure is deliberately unsupported
