...
2. Before presenting any artifact, render-verify the composed page the way the user will see it: load the saved HTML file itself in a browser (a direct file open or a headless-browser screenshot both work - artifacts are portable) and eyeball the rendered result - page background painted, every section's text visible with high contrast, nothing clipped - in both light and dark color schemes when you hand-wrote the palette. Verifying the inputs (source CSS, embedded images) is not verifying the page. Never render-verify through the Lavish session URL: loading it in your own browser takes over the user's live review tab - render the file directly instead.
...
4. Run `npx -y lavish-axi poll <html-file>` to long-poll for the user's annotations and queued prompts.
...
5. If poll returns feedback, apply the user's prompts. A `layout-warnings` prompt is an explicit repair request; apply every listed fix in one pass before saving, and let Lavish re-check it after a newer artifact load.
6. Apply human feedback, then poll again with `--agent-reply "<message>"` to reply in the browser and keep the loop going under the same foreground-or-verified-wake-path rule.
...
- Before presenting any artifact, render-verify the composed page the way the user will see it: load the saved HTML file itself in a browser (a direct file open or a headless-browser screenshot both work - artifacts are portable) and eyeball the rendered result - page background painted, every section's text visible with high contrast, nothing clipped - in both light and dark color schemes when you hand-wrote the palette. Verifying the inputs (source CSS, embedded images) is not verifying the page. Never render-verify through the Lavish session URL: loading it in your own browser takes over the user's live review tab - render the file directly instead.
