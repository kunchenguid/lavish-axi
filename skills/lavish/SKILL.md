...
- Before writing repeated artifact markup, run `npx -y lavish-axi registry search <need>`. Registered components own reusable HTML once. Run `npx -y lavish-axi recipe list` to find named compositions, then pass TOON content and component calls to `npx -y lavish-axi compose` instead of reproducing component HTML.
