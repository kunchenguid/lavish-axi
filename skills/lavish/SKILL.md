...
- Run `npx -y lavish-axi share <html-file> [--password <pw>]` to publish the artifact as a PUBLIC page on ht-ml.app and get back a visitable URL anyone can open. Local assets are inlined; remote refs load over the network. It returns the public url plus a secret update_key for managing the page later. Everything published is public; users can also publish from the browser chrome's overflow menu
