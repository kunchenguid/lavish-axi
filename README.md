...
- **Local-first state** - Session state stays under `$XDG_DATA_HOME/lavish-axi/` by default (`~/.local/share/lavish-axi/` when `XDG_DATA_HOME` is unset), or `LAVISH_AXI_STATE_DIR` when set. A pre-existing `~/.lavish-axi/` directory from an older version is migrated there automatically, once, the first time the server starts without `LAVISH_AXI_STATE_DIR` set.
...
| `lavish-axi server`      | `--verbose`           | Log session and watcher events to stderr; can also be enabled with `LAVISH_AXI_DEBUG=1`. Detached server output is appended to `$XDG_DATA_HOME/lavish-axi/server.log` (`~/.local/share/lavish-axi/server.log` by default, or `LAVISH_AXI_STATE_DIR/server.log` when set) for startup and crash diagnostics. |
