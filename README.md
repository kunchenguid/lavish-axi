...
- **Agent presence** - Receiving feedback and processing delivered work are independent. The browser keeps **Working...** visible while any delivered batch is unfinished, including during another receiving poll and after a page reload or reconnect. Human feedback actions stay available; the server queues each new submission for the next poll. The listening banner reflects whether a receiver is attached, even while the agent is working.
  Each non-terminal feedback result includes `session.feedback_id`. When that batch has finished, reply with `lavish-axi poll <html-file> --agent-reply "<outcome>" --feedback-id <id>`; the outcome can describe completion, failure, or cancellation. This concludes only that batch, so a late or repeated reply cannot clear newer work. Once every batch is concluded, presence returns to listening if a receiver is attached, otherwise waiting. Ending the session also clears its working indication.
  **Client migration:** integrations must carry each returned id into its completion reply. Replies without `--feedback-id` still appear in chat as progress but do not conclude work; starting a new poll no longer implies completion. Follow the updated poll's `next_step` command. Processing tracking belongs to the running server and resets on a server restart; it is not a monitor of the agent process.
...
- **Image attachments** - Attach reference images (PNG, JPEG, WebP) in either the Conversation composer or an annotation card by pasting, drag-dropping, or using its image picker; each image shows a thumbnail chip with upload, remove, retry, and error states.
  ...
  After a successful send, Conversation keeps a **Submitted** receipt with each attachment's name, image type, and thumbnail, including attachment-only messages and annotated images. These receipts survive reload and reconnect. If the image cannot be loaded or has expired, its receipt remains with **Preview unavailable**; history does not extend attachment-file retention. Older text-only history remains readable, but attachment references omitted by older versions cannot be reconstructed from that history.
...
| `lavish-axi poll`        | `--agent-reply "..."` | Show an agent reply in the browser chat before polling again. Without `--feedback-id`, this is a progress reply.                                                                                                                    |
| `lavish-axi poll`        | `--feedback-id <id>`  | With `--agent-reply`, conclude the corresponding delivered batch after completion, failure, or cancellation. Use the `session.feedback_id` returned by that poll.                                                                   |
