...
- **Portable artifacts** - The artifact runs in an iframe while Lavish injects a small SDK for annotations, snapshots, feedback controls, and render-time layout checks.
  ...
  Run `lavish-axi design` for the single source of agent-facing design guidance, the local Tailwind+DaisyUI build argument vectors, and the pinned Mermaid CDN snippet.
...
- **Agent presence** - The browser shows when no agent is listening, keeps queued feedback for the next successful `lavish-axi poll` send even across reloads, and only blocks human sends while the agent is working on delivered feedback; the agent's reply (`--agent-reply`) concludes that work and re-enables sends.
  ...
  In Codex code mode, the runtime guidance keeps the poll in one `functions.exec` callback cell: `yield_control()` releases the current model turn, long `tools.write_stdin` waits stay inside the cell, and one final `notify(...)` resumes the same agent when feedback arrives. That makes the wait model-free and avoids recurring Core `wait` calls. When those callback primitives are unavailable, the fallback is one poll attached to the active turn with the longest supported blocking wait. A detached `codex app-server` is deliberately not used to resume `CODEX_THREAD_ID`: it is a second runtime owner, not a callback into the currently loaded Codex client.
...
| `lavish-axi poll <html-file>`   | Long-poll until the user sends feedback or ends the session; detected layout issues wait in the user's Layout issues inbox and arrive only when queued. Leave no-timeout polls running, or re-run them if interrupted. Codex code mode uses one callback-owning exec cell so waiting stays model-free; other Codex runtimes keep one poll attached to the active turn. On `status: ended`, stop polling and do not reopen uninvited. |
...
| `lavish-axi design`             | Show agent-facing design guidance, the local Tailwind+DaisyUI build argument vectors, and the pinned Mermaid CDN snippet.                                                                                                                                                                                                                                                                                                            |
