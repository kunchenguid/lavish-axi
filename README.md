...
- **Reliable feedback delivery** - Every feedback response includes a stable `delivery_id` and is redelivered until the agent acknowledges it after processing with `lavish-axi poll <html-file> --ack <delivery_id>`.
  Acknowledging one delivery clears only that batch; prompts or layout failures that arrived while it was in flight remain queued for the next poll.
...
- **Agent presence** - The browser shows when no agent is listening, keeps queued feedback and proven severe layout failures until an agent acknowledges the delivery after processing, and only blocks human sends while the agent is working on delivered feedback.
  The no-timeout poll writes an immediate stderr banner and periodic stderr heartbeats while stdout stays reserved for the final response; if the poll is interrupted or its result is abandoned, re-run it to receive the same delivery again.
...
- **Session end etiquette** - Lavish tracks who ended a session: a human clicking **End session** (or **Send & End**) in the browser is a user-initiated end, while `lavish-axi end <html-file>` is agent-initiated.
  ...
  A final `feedback` response must be acknowledged after processing; the acknowledgement poll then reports `ended` and tells the agent to stop instead of reopening.
...
| `lavish-axi poll <html-file>`   | Long-poll until the user sends feedback, ends the session, or the browser proves a severe layout failure. Each batch is redelivered with the same `delivery_id` until processed and acknowledged with `--ack`; leave no-timeout polls running, or re-run them if interrupted. Codex guidance keeps polls attached to the active turn. On `status: ended`, stop polling and do not reopen uninvited. |
...
| `lavish-axi poll`        | `--ack <delivery-id>` | Acknowledge a feedback batch after processing it, then wait for more feedback.                                                                                                                                                      |
