...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  Queued annotation preview pills and chat history share a scrollable Conversation panel above a sticky composer, so long feedback queues do not push the text box or send controls off screen; before any chat arrives, the empty panel tells users to annotate the artifact or type below.
...
| `lavish-axi poll`        | `--agent-reply "..."` | Seed or show the agent's reply in the existing browser chat before polling.                                                                                                                                                         |
