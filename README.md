...
- **Editor theme** - The editor shell around the artifact reads an optional operator stylesheet, appended after the packaged one so plain `:root { ... }` overrides win without `!important`. It defaults to `theme.css` in the state directory, or `LAVISH_AXI_THEME_CSS` when set. The file is read on every request, so editing it and reloading the page is enough to see the change; a missing or unreadable file leaves the editor styled exactly as shipped.
  The annotation card that opens over the artifact is themed by the same file, but through its custom properties only - declare tokens such as `--bg`, `--bg-panel`, `--fg`, `--accent`, `--accent-ink`, `--border`, and `--font-sans` on `:root` and the card follows. The stylesheet's rules are never applied to the artifact itself, so its own design stays untouched.
