...
- **Reliable feedback delivery** - Every feedback response includes a stable `delivery_id` and is redelivered until the agent acknowledges it after processing with `lavish-axi poll <html-file> --ack <delivery_id>`.
  Acknowledging one delivery clears only that batch; prompts or layout warnings that arrived while it was in flight remain queued for the next poll.
...
- **Agent presence** - The browser shows when no agent is listening, keeps queued feedback and fresh layout warnings until an agent acknowledges the delivery after processing, and only blocks human sends while the agent is working on delivered feedback. The no-timeout poll writes an immediate stderr banner and periodic stderr heartbeats while stdout stays reserved for the final response; if the poll is interrupted or its result is abandoned, re-run it to receive the same delivery again.
...
| `lavish-axi poll <html-file>`   | Long-poll until feedback arrives. Each batch is redelivered with the same `delivery_id` until processed and acknowledged with `--ack`; on `status: ended`, stop polling and do not reopen uninvited.                                                                      |
