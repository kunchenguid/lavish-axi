...
- **Mobile reader mode** - On that same phone-width screen, scrolling down through the artifact also collapses the Lavish header and the Conversation dock, so the artifact gets the entire viewport; scrolling up, reaching the top of the artifact, the **Controls** button that appears while the chrome is collapsed, or Escape brings them straight back.
  Nothing is lost while the chrome is collapsed: a half-written comment, queued annotation pills, and chat history are all still there when it returns, and raising the conversation sheet always restores the chrome first.
  Lavish never collapses the chrome while the conversation sheet is open, while you are typing in the composer or an annotation card, while a menu, drawer, or dialog is open, or on desktop-width screens.
- **Keyboard shortcuts** - In the chrome composer, Enter sends queued prompts and Shift+Enter inserts a newline.
  ...
  On a narrow screen with the chrome collapsed by reader mode, Escape restores it from either the browser chrome or the artifact iframe, and the **Controls** button is the first tab stop on the page.
