...
- **Multiplexed event stream** - Primary deliverable is a domain-neutral foundation (`src/multiplexed-stream.js`) for one subscription covering many independently keyed streams, with durable retention, fencing, and backpressure. Lavish is the first consumer via a thin adapter (`src/lavish-stream-adapter.js`). Ordinary `poll` is unchanged for unclaimed reviews. Protocol: [docs/event-stream-protocol.md](docs/event-stream-protocol.md).
