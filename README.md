...
- **Full-page screenshots** - `lavish-axi screenshot <html-file>` renders the artifact in a throwaway headless Chrome/Chromium and writes one PNG spanning the document's full scroll height (a long screenshot), defaulting to `<name>.screenshot.png` next to the source at a 1280px-wide viewport (`--out`, `--width`).
  The same capture is available while reviewing from the browser chrome's overflow menu as **Export full-page PNG** (directly below **Export standalone HTML**), which downloads `<name>.screenshot.png` rendered at the same default viewport width.
  The editor export is WYSIWYG: it captures the artifact as you currently see it, including in-session state such as a language toggle, because the live document is serialized at export time and re-rendered for the capture. The CLI renders the file directly, so it always reflects the saved file, never an open editor session.
  The CLI is server-independent like `export` but needs a local Chrome or Chromium install; set `LAVISH_AXI_CHROME_PATH` to point at a specific binary.
...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  The browser chrome keeps editing actions in the overflow menu (copy path, reload artifact, copy DOM snapshot, export standalone HTML, export full-page PNG, publish link, end session), while the composer exposes **Send & End** beside **Send to Agent** to submit queued prompts and user-ended attribution together.
...
| `lavish-axi screenshot <html-file>` | Capture a full-page PNG of the rendered artifact - the document's entire scroll height, not just the viewport (a long screenshot). Renders the file in a throwaway headless Chrome/Chromium; needs no Lavish server but needs a local Chrome/Chromium install. Defaults to `<name>.screenshot.png` next to the source at a 1280px-wide viewport. |
...
| `lavish-axi screenshot`  | `--out <path>`        | Write the PNG to a specific path instead of `<name>.screenshot.png` next to the source.                                                                                                                                             |
| `lavish-axi screenshot`  | `--width <px>`        | Render at a specific viewport width (default 1280) before capturing the full document height. Set `LAVISH_AXI_CHROME_PATH` to point at a specific Chrome/Chromium binary.                                                           |
