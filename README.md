...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  Composer feedback becomes a visible queued pill and is stored in that tab before the text box clears. It stays queued until the server acknowledges it. If acknowledgment takes longer than 10 seconds, Lavish keeps the queue and shows persistent recovery guidance.
