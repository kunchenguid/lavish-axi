...
Lavish Editor opens agent-generated HTML files in a local browser, lets you pinpoint elements and selected text, and sends grounded feedback back to the agent. The Dealernet launcher supplies `--ticket <ISSUE>` so the demand remains visible in the single top bar on every viewport.
...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  Artifacts that need workflow actions can call `window.lavish.registerActionPanel(config, handler)` to add a safe, declarative section above Conversation in the same sidebar, then call `window.lavish.updateActionPanel(id, state)` to update its summary, status, and disabled actions. The closed `lavish-action-panel-v1` descriptor accepts text, textarea fields, and `primary`, `neutral`, or `danger` actions; artifact strings are rendered as text, never HTML.
  The handler receives only `{ actionId, values }`. To submit the queued payload and close the current review in one server request, await `window.lavish.sendQueuedPrompts({ endSession: true })` and check its `{ ok, error? }` result. Calling `sendQueuedPrompts()` without options remains non-terminal.
  Queued annotation preview pills and chat history share the expandable Conversation section above a sticky composer, so long feedback queues do not push the text box or send controls off screen. Gate and Conversation stay simultaneously visible, and Conversation starts open.
  On viewports up to 860 px the complete artifact comes first and the same panel follows it in the document flow, producing one vertical page scroll instead of competing nested scrollers.
...
| `lavish-axi <html-file> [--ticket <ISSUE>]` | Open or resume a Lavish Editor session, optionally showing the validated demand identifier in the top bar. The open-time layout gate remains enabled by default. Unresolved layout issues are preserved. Refuses to reopen a user-ended session unless `--reopen` is passed.                                                                 |
