...
- **Server cleanup** - The detached server stops after the last session ends when nothing is connected, or after `LAVISH_AXI_IDLE_TIMEOUT_MS` (default 30 minutes) with no browser, poll, or event-stream subscription connections.
...
- **Network binding** - The server binds to loopback (`127.0.0.1`) by default. Set `LAVISH_AXI_HOST` to bind elsewhere; a wildcard (`0.0.0.0` or `::`) binds every interface. Binding beyond loopback exposes an unauthenticated server that can read and serve arbitrary local files to anything that can reach it, so only do so on a trusted network. Network position is never authentication for the event stream - that surface requires a home capability from a mode-0600 `--home-file`. Set `LAVISH_AXI_LINK_HOST` to control the hostname written into generated session links (defaults to the bind address, or loopback when bound to a wildcard).
- **Multiplexed event stream** - Supervisors can use one durable subscription for many independently keyed streams, with fencing and backpressure; Lavish reviews are the first supported stream type. Ordinary `poll` is unchanged for unclaimed reviews. Defaults are a 1 MB payload, 256 unacknowledged events per stream, 4,096 per consumer, seven-day retention, and a 60-second lease. Override them with `LAVISH_AXI_EVENT_MAX_PAYLOAD_BYTES`, `LAVISH_AXI_EVENT_MAX_UNACKED_STREAM`, `LAVISH_AXI_EVENT_MAX_UNACKED_CONSUMER`, `LAVISH_AXI_EVENT_RETENTION_MS`, and `LAVISH_AXI_EVENT_LEASE_TTL_MS`; the older `..._REVIEW` and `..._HOME` unacknowledged-limit names remain aliases. Protocol: [docs/event-stream-protocol.md](docs/event-stream-protocol.md).
...
| `lavish-axi capabilities`               | Print machine-readable protocol capabilities, including the domain-neutral `protocol.event_stream` range (`--json` for strict JSON).                                                                                                                                                                                                         |
| `lavish-axi home init`                  | Mint a mode-0600 home capability file for a supervisor home. The token stays in the file; pass it via `--home-file` only.                                                                                                                                                                                                                    |
| `lavish-axi review claim\|retire\|list` | Claim or retire a review on the multiplexed stream (Lavish adapter), or list claims.                                                                                                                                                                                                                                                         |
| `lavish-axi events subscribe`           | One NDJSON subscription covering every review claimed by a home. See [docs/event-stream-protocol.md](docs/event-stream-protocol.md).                                                                                                                                                                                                         |
...
| `lavish-axi capabilities`     | `--json`              | Emit strict JSON instead of the default AXI output.                                                                                                                                                                                 |
| `lavish-axi home init`        | `--root <path>`       | Set the supervisor home root; required.                                                                                                                                                                                             |
| `lavish-axi home init`        | `--file <path>`       | Write the capability to a specific path instead of `<root>/.lavish-home.json`.                                                                                                                                                      |
| `lavish-axi review claim`     | `--home-file <path>`  | Read the home capability from this mode-0600 file; required.                                                                                                                                                                        |
| `lavish-axi review claim`     | `--generation <n>`    | Set the positive fencing generation; required.                                                                                                                                                                                      |
| `lavish-axi review claim`     | `--review <id\|file>` | Claim a review by session key or artifact path; required.                                                                                                                                                                           |
| `lavish-axi review retire`    | `--home-file <path>`  | Read the home capability from this mode-0600 file; required.                                                                                                                                                                        |
| `lavish-axi review retire`    | `--generation <n>`    | Set the positive fencing generation; required.                                                                                                                                                                                      |
| `lavish-axi review retire`    | `--review <id\|file>` | Retire a review by session key or artifact path; required.                                                                                                                                                                          |
| `lavish-axi review list`      | `--home-file <path>`  | List the reviews claimed by this home capability; required.                                                                                                                                                                         |
| `lavish-axi events subscribe` | `--home-file <path>`  | Read the home capability from this mode-0600 file; required.                                                                                                                                                                        |
| `lavish-axi events subscribe` | `--generation <n>`    | Set the positive fencing generation; required.                                                                                                                                                                                      |
| `lavish-axi events subscribe` | `--cursor <n>`        | Resume after a prior log position; defaults to the retained beginning.                                                                                                                                                              |
