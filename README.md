...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  Question wrappers marked with `data-lavish-question` show a visible **Queued** state while their answer is waiting to be sent; replacing that answer keeps one marker, and removing or sending it clears the marker.
