...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  On wider screens, the conversation and the queued notes share one scrollable Conversation panel above a sticky composer, so long feedback queues do not push the text box or send controls off screen. Use the chevron in its header to collapse it into a narrow rail and reclaim the artifact width; the choice stays with that review tab across reloads. While it is collapsed the chevron carries a dot when prompts are queued or a reply arrives, and the panel reopens itself whenever something needs your attention there - a failed or unacknowledged send, a takeover notice, or an update notice.
