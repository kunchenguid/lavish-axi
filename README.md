...
- **Local-first state** - Session state stays under `~/.lavish-axi/` by default, or `LAVISH_AXI_STATE_DIR` when set.
  The same `state.json` also holds a top-level `config` object that stores the device-wide Lavish theme preference set via `lavish-axi config theme <system|light|dark>`. The preference is preserved through every session write, so a `config` change survives the next session upsert.
...
- **Theming** - Lavish ships a standard light/dark theme for the editor chrome (toolbar, panels, send dialogs), with the device preference defaulting to `system` so the chrome live-follows `prefers-color-scheme`; `light` and `dark` pin it.
  Artifact content is never overridden at runtime: Lavish does not inject a content stylesheet or theme opt-out, so artifacts stay portable and render identically when opened directly.
  Instead, the `home` and `design` CLI outputs include a `theme_preference.directive` that the agent reads while generating the artifact and bakes into the artifact's own CSS, Tailwind theme, or DaisyUI `data-theme`. `system` is resolved against the current OS appearance at the moment the command runs (via `osascript` on macOS, the `AppsUseLightTheme` registry value on Windows, and `gsettings` on Linux) and surfaces as a concrete "currently in light/dark mode" directive; on platforms that cannot be queried it falls back to a `prefers-color-scheme` CSS hint, since a statically baked theme cannot live-track later OS toggles.
...
| `lavish-axi config [theme]`     | Read or set the device-wide Lavish theme preference. Bare `config` prints the full config; `config theme` prints just the theme; `config theme <system\|light\|dark>` sets it. The preference drives the editor chrome and is surfaced to agents as artifact generation guidance. |
