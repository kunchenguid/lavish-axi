...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  When the chat is empty, the panel tells the user the agent has not sent a message yet and points them to annotating the artifact or typing below.
...
- **Agent presence** - The browser shows when no agent is listening, keeps queued feedback and fresh layout warnings for the next successful `lavish-axi poll` send even across reloads, and only blocks human sends while the agent is working on delivered feedback. Agents can pass `--agent-reply` on the first poll to seed the empty chat with a one-line summary. The no-timeout poll writes an immediate stderr banner and periodic stderr heartbeats while stdout stays reserved for the final response; if the poll is interrupted or times out, re-run it because queued feedback is never lost.
...
| `lavish-axi poll`        | `--agent-reply "..."` | Seed or show an agent message in the browser chat before polling.                                                                                                                                                                   |
