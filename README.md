...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  Queued annotation preview pills and chat history share the expandable Conversation section above a sticky composer, so long feedback queues do not push the text box or send controls off screen. On compact Gate reviews the action section stays first and Conversation starts collapsed.
