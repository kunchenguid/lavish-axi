...
- **Layout issue lifecycle** - Each issue is identified by a stable fingerprint of the diagnostic rule, the normalized target identity, the viewport class, and the authored page, so repeat detections on that page update one record instead of inflating the count.
...
- **Export and sharing** - `lavish-axi export` writes `<name>.export.html` from the session entry file by inlining its local assets only, stripping the annotation SDK, and leaving remote CDN/font references as links that still need network access. Export and share remain entry-only even when the reviewer is currently viewing an authored sibling; they do not package a whole site or expose page proofs.
  ...
  Bundling never fetches remote URLs, exported and shared HTML receives no Lavish CSP, local reads stay confined and size-capped, and absolute `file://` paths outside safe inlined asset references are redacted before output.
