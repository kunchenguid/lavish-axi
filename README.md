...
The skill is a short discovery stub. It sends the agent to `lavish-axi --help`, `design`, and `playbook` for current instructions, so an installed copy cannot go stale against a newer CLI.
It uses the `lavish-axi` command on `PATH`; a managed environment may provide that command as a pinned wrapper around a reviewed package version. If the command is missing, the skill tells the agent to stop instead of fetching an unpinned package.
...
- **Export and sharing** - `lavish-axi export` writes `<name>.export.html` by inlining local assets only, stripping the annotation SDK, and leaving remote CDN/font references as links that still need network access.
  ...
  Agents run `share` only when the user explicitly asks to publish or share the artifact outside the local machine.
...
- **Network binding** - The review server is loopback-only by default. Set `LAVISH_AXI_TAILSCALE=1` to also listen on this machine's Tailscale IPv4 address and use its MagicDNS name for phone-ready session links. Lavish never listens on `0.0.0.0` or every interface. When the opt-in is enabled but Tailscale is absent or down, Lavish stays loopback-only and prints no phone URL. If MagicDNS is unavailable or its address cannot be bound after brief retries, Lavish warns that phone access is unavailable and remains loopback-only. Tailscale access exposes an unauthenticated server that can read and serve arbitrary local files to devices on the tailnet, so enable it only on a trusted tailnet. Any explicit `LAVISH_AXI_HOST` overrides Tailscale binding; wildcard values are restricted to loopback, while a non-wildcard value selects that one concrete bind address. `LAVISH_AXI_LINK_HOST` controls the link host when Tailscale access is disabled.
