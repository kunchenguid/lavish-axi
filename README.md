...
- **Tailnet access** - `lavish-axi tailnet` serves live sessions to your other Tailscale devices (a phone or tablet on the same tailnet) without changing the bind address: the server stays on loopback while `tailscale serve --bg --https=8443 http://127.0.0.1:<port>` has tailscaled terminate TLS on the tailnet and proxy locally. The serve config is scoped to its own HTTPS port (default `8443`, override with `LAVISH_AXI_TAILNET_HTTPS_PORT`), so an existing `tailscale serve` config on `:443` is never touched, and `--off` clears only Lavish's port. While enabled, the device's MagicDNS hostname is added to the Host allowlist (below) and session output includes a `tailnet_url` like `https://<magicdns>:8443/session/<key>`; annotations and `lavish-axi poll` feedback work exactly as on the desktop. Requires the Tailscale CLI, a signed-in device, and MagicDNS. Access control is your tailnet's WireGuard device authentication - links do not work off the tailnet, and public exposure via `tailscale funnel` is deliberately unsupported because session keys are derived from file paths and the server is unauthenticated.
- **Allowed hosts** - To defend against DNS rebinding, the server rejects (`403`) any request whose `Host` header is missing or not one it answers to: the loopback names (`127.0.0.1`, `::1`, `localhost`) plus the configured bind and link host, and when Tailnet access is enabled the device's MagicDNS hostname (see Tailnet access above). If you reach the server under another name - a wildcard bind accessed by LAN IP, a reverse-proxy hostname, or an extra interface - list those names in `LAVISH_AXI_ALLOWED_HOSTS` (whitespace-separated) to allow them. Behind a reverse proxy, the forwarded `X-Forwarded-Host` is validated against the same list, so add your public hostname there and have the proxy send it. Set `LAVISH_AXI_ALLOWED_HOSTS` to `*` to disable the check entirely (only when the server sits behind your own authentication or proxy).
...
| `lavish-axi tailnet`            | Serve live sessions to your other Tailscale devices (phone, tablet), private to your tailnet: the server stays loopback-bound while `tailscale serve` proxies a dedicated HTTPS port (default `8443`, `LAVISH_AXI_TAILNET_HTTPS_PORT`) to it, and session output gains a `tailnet_url`. `--status` shows state; `--off` clears only Lavish's port-scoped serve config. Public exposure is deliberately unsupported. |
...
| `lavish-axi tailnet`     | `--status`            | Show whether tailnet access is enabled, the MagicDNS hostname, and per-session `tailnet_url`s.                                                                                                                                      |
| `lavish-axi tailnet`     | `--off`               | Clear Lavish's port-scoped `tailscale serve` config and restart the server without the tailnet hostname allowlisted.                                                                                                                |
