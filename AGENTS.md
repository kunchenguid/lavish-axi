...
7. `lavish-axi poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
   Queued prompts or layout warnings - including prompts queued before an ended session - are returned immediately with a stable `delivery_id` and mark the session as observed by an agent; otherwise the poll marks the session actively listening and long-polls on an `EventEmitter` until a `feedback` or `ended` event fires. The agent acknowledges a processed batch on its next poll with `--ack <delivery_id>`; abandoned and concurrent polls receive the same delivery until then.
