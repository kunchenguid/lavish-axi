...
CLI execution detects Codex dynamically from `CODEX_SANDBOX` or `CODEX_THREAD_ID` and selects Codex-specific foreground polling guidance; `src/skill.js` explicitly requests the agent-neutral `static` variant because generated `SKILL.md` has no invoking runtime.
