...
7. `lavish-axi poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
   ...
   Each poll first releases any older waiting poll for the same key with `status: "superseded"`, before its own drain rather than only on the waiting branch: emitter listeners fire oldest-first and `takeFeedback` drains destructively, so an abandoned earlier-turn poll left listening swallows the next prompt into a log nobody reads, including when this poll returns queued feedback immediately.
   Only the newest poll delivers, which is also what bounds real listeners to one per key.
...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation, with no in-memory cache - acceptable because writes are infrequent. Every read-modify-write goes through `withLock`; an in-process promise mutex suffices only because all mutations flow through the single server process (the CLI only reads), so a new mutating path must stay server-side. `writeState` publishes by `rename` from a tmp file named per write with pid plus random bytes: the rename keeps direct readers from seeing torn JSON, and the unique name keeps two briefly-coexisting servers (a version-handshake upgrade) from interleaving one tmp file and renaming the torn result into place.
