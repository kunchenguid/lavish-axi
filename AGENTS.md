...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation. There's no locking - acceptable because writes are infrequent and serialized through the single server process. The one in-memory exception is `feedbackTakes`: a `Map` that deduplicates concurrent `takeFeedback` calls so two polls arriving simultaneously for the same session share one promise and produce one state write rather than two conflicting ones.
