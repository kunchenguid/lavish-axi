...
1. `lavish-axi <file.html>` (`openCommand`) -> POST `/api/sessions` -> `SessionStore.upsertSession` -> the CLI opens the returned session URL in the browser.
   ...
   On `--reopen`, `upsertSession` carries forward any pending `feedback_delivery` (stripping `session_ended` and `ended_by` so the delivery no longer implies the session is over) and recent acknowledged delivery ids, so an unacknowledged delivery survives the reopen and duplicate `--ack` calls remain idempotent.
...
5. The injected SDK runs the render-time layout audit (detection kinds and user-facing gate behavior: README's Layout warnings and Open-time layout gate bullets) and posts findings to the chrome, which POSTs them to `/api/:key/layout-warnings`.
   ...
   `takeFeedback` leases layout warnings alongside human prompts under a stable `delivery_id`; it redelivers that lease until explicit acknowledgement, then remembers delivered `kind:selector` keys for repeat detection. Acknowledgement clears only the leased prompt prefix and unchanged warning set, so feedback queued while a delivery is in flight survives.
...
7. `lavish-axi poll <file.html>` (`pollCommand`) optionally `POST`s `/api/:key/ack` first (when `--ack <delivery_id>` is passed, triggering `SessionStore.acknowledgeFeedback`), then hits `GET /api/poll`.
   Queued prompts or layout warnings - including prompts queued before an ended session - are returned immediately with a stable `delivery_id` and mark the session as observed by an agent; otherwise the poll marks the session actively listening and long-polls on an `EventEmitter` until a `feedback` or `ended` event fires. Abandoned and concurrent polls receive the same delivery until it is explicitly acknowledged. Sending a recently acknowledged `delivery_id` again is safe: `acknowledgeFeedback` returns `"already_acknowledged"`, and the server responds 200 without emitting a presence change (the delivery is already cleared). Sending an unknown or expired `delivery_id` returns `"mismatch"`, and the server responds 409 `STALE_DELIVERY_ID`; the CLI converts this to a `VALIDATION_ERROR` with guidance to re-run poll without `--ack`.
   ...
   A `status: "ended"` response carries `ended_by`; the final `status: "feedback"` batch carries the same signal via `session_ended: true` plus `ended_by`. If send-and-end adds newer prompts while an older delivery is in flight, that older lease deliberately omits the ended signal so its `next_step` keeps polling; after acknowledgement, the newer final batch is delivered with the ended signal instead of being cleared unseen.
   ...
   If SIGINT or SIGTERM interrupts a no-timeout poll, the CLI writes re-run guidance to stderr and exits with the conventional signal code; unacknowledged feedback persists, so re-running the same poll is safe even when the prior process already received the response.
8. The `/events/:key` SSE stream emits `agent-presence` states: `waiting` before any poll has attached, `listening` while one is active, and `working` after a poll has delivered feedback and released; the chrome allows queued feedback while waiting or listening and blocks sends only while working.
   `--agent-reply` posts a chat message into the session before polling, rendered in the browser conversation panel via the same stream. When paired with `--ack`, the CLI sends the `delivery_id` with the reply and the store remembers recent reply delivery ids, so retrying the full ack-and-reply command cannot duplicate the browser chat message.
...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation. Its in-memory `stateMutation` promise queue serializes every state-changing read-modify-write within the server process so overlapping prompt, warning, delivery, acknowledgement, end, reopen, and reply requests cannot clobber one another. `feedbackTakes` additionally deduplicates concurrent `takeFeedback` calls so two polls for the same session share one delivery promise. This is deliberately a per-process guard, not a cross-process file lock; the detached server remains the sole state writer.
...
- The in-iframe layout audit (`src/artifact-sdk.js`) has four easy-to-reintroduce failure modes, all covered by dedicated tests:
  ...
  - `SessionStore` marks a re-reported layout warning `persistent: true` once its `kind:selector` key has been acknowledged by the agent (tracked in `session.delivered_layout_warning_keys`, independent of the active `layout_warnings` field). `cli.js`'s `createFeedbackNextStep` softens its guidance - permitting the agent to proceed to the human instead of looping fixes and reloads - once every current warning is either `persistent` or below `error` severity (currently only `overlapping-text`, since it stays heuristic even after fragment-aware matching).
  - `layoutWarningsSignature` (`src/session-store.js`) must compare layout warning arrays **order-independently**: it sorts each warning's keys alphabetically before serializing, then sorts the resulting strings. A naive `JSON.stringify(warnings)` would fail to clear warnings after acknowledgement whenever the browser re-sends them in a different order. Any reimplementation must preserve both sort steps.
