...
Hovering a rendered Mermaid diagram inside a `.mermaid` container shows an "Edit as whiteboard" button (SDK-injected, `data-lavish-ui`), which asks the chrome to open a full-viewport overlay hosting `/whiteboard-frame` in its own sandboxed iframe (`allow-scripts allow-popups`, no `allow-same-origin`).
...
The frame has no server access: the chrome fetches diagram sources (`GET /api/:key/mermaid-sources`, extracted from the artifact file on disk by `src/mermaid-source.js` in `.mermaid` document order) and saved scenes, passes them over postMessage, and performs all writes.
Scenes autosave (debounced) through `PUT /api/:key/whiteboard/:index` into per-diagram sidecar files under `<state-dir>/whiteboards/<key>/` (`src/whiteboard-store.js`) - never into `state.json`, which is rewritten wholesale on every store operation.
