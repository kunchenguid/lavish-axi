...
The chrome bootstraps the inbox from `initialLayoutWarnings` in the session JSON and re-syncs over SSE, so the inbox survives a browser refresh or reconnect; only per-chrome view state (drawer selection) lives in `sessionStorage`, keyed per session so it cannot leak across artifacts.
