...
- Table-cell annotations attach `src/table-cell.js`'s semantic row/column names as `target` only. The clicked element's own `selector`, `tag`, and `text` keep describing that element, because the on-screen highlight outlines exactly what was clicked. Both coordinates stay silent rather than name a row or column they cannot prove, because a confidently wrong name reads as authoritative and is worse than none: a rowspan anywhere (including `rowspan="0"`, which spans to the end of the row group) means DOM order is not rendered order, so it suppresses the column label and the positional row heading alike - only a declared `scope="row"` heading survives it - and a row whose colspans do not sum to the header's, or a cell straddling a grouped header, names no column. Resolving a cell walks its whole table, so `context()` computes the target only under `{ table: true }`, which the annotation card passes and `snapshot()` - which calls `context()` for every element in the document - deliberately does not.
