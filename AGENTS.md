...
1. `lavish-axi <file.html>` (`openCommand`) -> POST `/api/sessions` -> `SessionStore.upsertSession` -> server returns `http://127.0.0.1:PORT/session/<key>` and the CLI calls `open` to launch the browser.
   ...
   `SessionStore.endSession(key, endedBy)` records who ended a session: `"user"` for the browser chrome's keyed `/api/:key/end` (both "End session" and "Send & end session" hit this route) versus `"agent"` for the CLI's file-based `/api/end` (`lavish-axi end <file>`). Idle self-shutdown never calls `endSession` at all - it only stops the server process, leaving session status untouched in `state.json`, so there is no third `ended_by` case to track there.
