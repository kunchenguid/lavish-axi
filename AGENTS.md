...
6. User actions in the iframe `postMessage` to the chrome (queue prompts, request snapshot, end session, toggle annotate/explore mode).
   ...
   The conversation panel keeps chat bubbles and queued prompt preview pills in a scrollable region above the sticky composer, so a long queue cannot push the textarea or send controls out of view; when no chat bubbles exist, CSS renders a short empty-state hint that points the user to annotate the artifact or type below.
...
`src/skill.js` renders the installable Agent Skill from the same home output, rewriting command examples to non-interactive `npx -y lavish-axi ...` invocations, omitting live session state, adding skill-only workflow guidance such as a first-poll `--agent-reply` context summary, and including Hermes Agent frontmatter metadata for categorization.
