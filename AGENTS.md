...
7. `lavish-axi poll <file.html>` (`pollCommand`) optionally `POST`s `/api/:key/ack` first (when `--ack <delivery_id>` is passed, triggering `SessionStore.acknowledgeFeedback`), then hits `GET /api/poll`.
   Queued prompts or layout warnings - including prompts queued before an ended session - are returned immediately with a stable `delivery_id` and mark the session as observed by an agent; otherwise the poll marks the session actively listening and long-polls on an `EventEmitter` until a `feedback` or `ended` event fires. Abandoned and concurrent polls receive the same delivery until it is explicitly acknowledged.
