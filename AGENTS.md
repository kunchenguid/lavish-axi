...
Each client registers independently and a client that cannot be registered must be _reported_, never thrown: `linkCursorLocalPlugin` returns `unsupported` instead of raising so one unlinkable client cannot abort the command or the other clients. On Windows it links with a **junction**, because a directory symlink there needs a privilege an ordinary account lacks (Developer Mode or elevation) - a plain `symlinkSync` would fail with EPERM for most Windows users. Tests that assert on a real on-disk link skip when symlink creation is unavailable; the link-creation logic itself stays covered everywhere through injected operations.
