...
Every observation is stamped with an **artifact revision**: `session.artifact_revision`, incremented by the `/artifact/:key/index.html` route because serving that document is the one point where a new artifact load is certain. Every transition that needs "a newer successful artifact load" (resolved, recurring, dismissal expiry) keys off it.
