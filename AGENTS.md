...
- A session stays owned by its one canonical entry file. Authored sibling HTML pages must be root-contained, every sibling source read re-runs lexical and `realpath` containment, and a page proof is never filesystem permission. Export, share, watcher defaults, CLI session identity, and open/poll/end stay entry-only. [Things to know when editing](docs/invariants.md#things-to-know-when-editing).
- Feedback batches, layout warnings, persisted review state, and whiteboards are page-scoped. Capture the authenticated page before every await, and never let one page's pass, send, or save land on another page. [Things to know when editing](docs/invariants.md#things-to-know-when-editing).
...
- Path-addressed `/artifact/*` responses omit `frame-ancestors`, so chrome authentication is the boundary: a protocol-1 document answers no `lavish:challenge` unless it carries the `signChromeAuth` MAC for that document's nonce and comes from the injected SDK script's origin. The legacy virtual entry and the export response keep `frame-ancestors 'self'`. [Things to know when editing](docs/invariants.md#things-to-know-when-editing).
