...
1. `lavish-axi <file.html>` (`openCommand`) -> POST `/api/sessions` -> `SessionStore.upsertSession` -> the CLI opens the returned session URL in the browser.
   ...
   On `--reopen`, `upsertSession` carries forward any pending `feedback_delivery` (stripping `session_ended` and `ended_by` so the delivery no longer implies the session is over) and `last_acknowledged_delivery_id`, so an unacknowledged delivery survives the reopen and duplicate `--ack` calls for the previous delivery remain idempotent.
...
7. `lavish-axi poll <file.html>` (`pollCommand`) optionally `POST`s `/api/:key/ack` first (when `--ack <delivery_id>` is passed, triggering `SessionStore.acknowledgeFeedback`), then hits `GET /api/poll`.
   Queued prompts or layout warnings - including prompts queued before an ended session - are returned immediately with a stable `delivery_id` and mark the session as observed by an agent; otherwise the poll marks the session actively listening and long-polls on an `EventEmitter` until a `feedback` or `ended` event fires. Abandoned and concurrent polls receive the same delivery until it is explicitly acknowledged. Sending the same `--ack <delivery_id>` a second time is safe: `acknowledgeFeedback` returns `"already_acknowledged"` when the id matches `last_acknowledged_delivery_id`, and the server responds 200 without emitting a presence change (the delivery is already cleared).
