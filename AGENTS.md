...
5. The injected SDK runs the render-time layout audit (detection kinds and user-facing gate behavior: README's Layout warnings and Open-time layout gate bullets) and posts findings to the chrome, which POSTs them to `/api/:key/layout-warnings`.
   ...
   The safety timeout (`layoutGateMaxHoldMs`, configurable server-side via `LAVISH_AXI_LAYOUT_GATE_MAX_HOLD_MS`/`resolveLayoutGateMaxHoldMs` and threaded into the chrome bootstrap by `createChromeHtml`; user-facing default and cap: README's Open-time layout gate bullet) reveals with a may-have-layout-issues banner when the audit runs long.
   Because the artifact iframe shares the chrome's main thread, a heavy artifact can starve the audit past that deadline and trip a false-positive banner, so after a timeout reveal `handleLayoutWarningsForGate` tracks `layoutGateTimedOut` (reset on re-arm) and, like the manual-bypass path, keeps reconciling the banner against later audits - clearing it when a late audit lands clean and keeping it when errors remain.
...
6. User actions in the iframe `postMessage` to the chrome. Queued prompts live in tab `sessionStorage`; unsent prompts sharing the SDK-internal `_lavishQueueKey` replace each other, that field is stripped before POSTing collected prompts to `/api/:key/prompts`, and sent prompts are removed only after a successful response.
   A send first requests a DOM snapshot from the sandboxed iframe and submits once the reply arrives; because that reply can be starved on the same busy shared main thread, `armSubmitSnapshotFallback` posts the prompts without the snapshot after `submitSnapshotTimeoutMs` (4s) and shows a hint, so a wedged bridge cannot turn Send into a silent no-op (a late reply for an already-submitted send is ignored via `awaitingSubmitSnapshot`).
