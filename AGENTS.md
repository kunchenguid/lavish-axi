...
Frames have no server access: the SDK relays their `lavish-whiteboard:*` messages to the chrome (`lavish:whiteboardRelay`, stamping the diagram index from which frame sent it), and the chrome fetches diagram sources (`GET /api/:key/mermaid-sources`, extracted from the artifact file on disk by `src/mermaid-source.js` in `.mermaid` document order) and saved scenes, passes them back over postMessage, and performs all writes - serving every frame concurrently, keyed by index.
