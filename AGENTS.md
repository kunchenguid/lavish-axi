...
5. The injected SDK runs the render-time layout audit (detection and user-facing gate behavior: README's Layout failures and Open-time layout gate bullets) and posts only stable, proven severe findings to the chrome, which POSTs them to `/api/:key/layout-warnings`.
   ...
   `takeFeedback` leases severe findings alongside human prompts under a stable `delivery_id`; it redelivers that lease until explicit acknowledgement, then remembers root identity including kind, selector, axis, viewport class, and magnitude for repeat detection. Acknowledgement clears only the leased prompt prefix and unchanged warning set, so feedback queued while a delivery is in flight survives.
