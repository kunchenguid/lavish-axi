...
5. The injected SDK also runs a render-time layout audit after `document.fonts.ready` and a brief `ResizeObserver` settle window.
   ...
   The gate re-arms on hot reload and reveals after the next clean audit; **Show anyway** and a bounded safety timeout reveal with a may-have-layout-issues banner instead of blocking review indefinitely.
   The safety timeout is `layoutGateMaxHoldMs` (default 12000ms, client-clamped to 60000ms), configurable server-side through `LAVISH_AXI_LAYOUT_GATE_MAX_HOLD_MS` (`resolveLayoutGateMaxHoldMs`) and threaded into the chrome via the `lavish-session` bootstrap by `createChromeHtml`.
   Because the artifact iframe is same-origin with the chrome and shares one main thread, a heavy artifact can starve the audit past that deadline and trip a false-positive banner, so after a timeout reveal the chrome keeps reconciling the banner against later audits: `handleLayoutWarningsForGate` tracks `layoutGateTimedOut` (reset on re-arm) and, like the manual-bypass path, clears the banner when a late audit lands clean and keeps it when errors remain.
...
6. User actions in the iframe `postMessage` to the chrome (queue prompts, request snapshot, end session, toggle annotate/explore mode).
   ...
   A send first requests a DOM snapshot from the sandboxed iframe (`requestSnapshot("submit")`) and submits once the `lavish:snapshot` reply arrives; because that reply comes from the same injected SDK that can be starved on a busy shared main thread, `armSubmitSnapshotFallback` posts the prompts without the DOM snapshot after `submitSnapshotTimeoutMs` (4s) and shows a hint, so a wedged bridge cannot turn Send into a silent no-op (a late reply for an already-submitted send is ignored via `awaitingSubmitSnapshot`).
